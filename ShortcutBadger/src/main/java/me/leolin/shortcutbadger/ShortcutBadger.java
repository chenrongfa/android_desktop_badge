...
import me.leolin.shortcutbadger.impl.CoolpadHomeBadger;
...
import me.leolin.shortcutbadger.impl.Qihoo360HomeBadger;
import me.leolin.shortcutbadger.impl.QikuHomeBadger;
...
public final class ShortcutBadger {
	...
	private static final List<Class<? extends Badger>> BADGERS = new LinkedList<Class<?
			extends Badger>>();
	...
	/**
	 * Tries to update the notification count, throw a {@link ShortcutBadgeException} if it
	 * fails
	 *
	 * @param context    Caller context
	 * @param badgeCount Desired badge count
	...
	/**
	 * Tries to remove the notification count, throw a {@link ShortcutBadgeException} if it
	 * fails
	 *
	 * @param context Caller context
	 */
	...
	/**
	 * Whether this platform launcher supports shortcut badges. Doing this check causes the
	 * side
	...
	public static boolean isBadgeCounterSupported(Context context) {
		// Checking outside synchronized block to avoid synchronization in the common case
		// (flag
			...
			synchronized (sCounterSupportedLock) {
				...
				if (sIsBadgeCounterSupported == null) {
					...
					for (int i = 0; i < SUPPORTED_CHECK_ATTEMPTS; i++) {
						try {
							Log.i(LOG_TAG, "Checking if platform supports badge counters, " +
									"attempt "
							...
							// Keep retrying as long as we can. No need to dump the stack
							// trace here
							// because this error will be the norm, not exception, for
							// unsupported
							// platforms. So we just save the last error message to display
							// later.
	 ...
	 * @param badgeCount
	 */
	public static void applyNotification(Context context, Notification notification,
										 int badgeCount) {
	...
	private static boolean initBadger(Context context) {
		...
		ResolveInfo resolveInfoDefault = context.getPackageManager().resolveActivity(intent,
				PackageManager.MATCH_DEFAULT_ONLY);
	 ...
	 */
	private static boolean isLauncherVersionSupported(Context context, String
			currentHomePackage) {
	 ...
	 */
	private static void validateInfoList(ResolveInfo
												 defaultActivity, List<ResolveInfo> resolveInfos) {
